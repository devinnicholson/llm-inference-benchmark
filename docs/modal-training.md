...
Training 060 extends this smoke to a merged r5 follow-up. The remaining
repeat-count target is r8.

# Training 060: Qwen 0.5B Ultra-Long Merged r5

Training 060 adds a three-repeat ultra-long Qwen chunk and merges it with the
Training 059 r2 smoke. This turns the longer-context checkpoint from a smoke
test into a five-repeat follow-up while keeping the same clean prompt-control
design.

## Goal

Check whether the ultra-long result remains favorable after more repeats before
spending another chunk to reach r8.
...
modal run modal_app.py --mode vllm-prefix-cache-isolated-neutral-warmup \
  ...
  --prefix-cache-shared-profile shared_prefix_ultra_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_ultra_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-chunk-r3-seed1401
```
...
```bash
modal run modal_app.py --mode vllm-prefix-cache-isolated-merge \
  --prefix-cache-isolated-merge-dirs results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-smoke-r2,results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-chunk-r3-seed1401 \
  --prefix-cache-shared-profile shared_prefix_ultra_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_ultra_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-merged-r5

modal run modal_app.py --mode vllm-prefix-cache-isolated-stability-summary \
  --prefix-cache-isolated-metrics-dir results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-merged-r5 \
  --prefix-cache-shared-profile shared_prefix_ultra_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_ultra_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-merged-r5-summary

python3 scripts/build_prefix_cache_study_table.py \
  --summary-json results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-merged-r5-summary/prefix-cache-isolated-stability-summary.json \
  --output-dir results/prefix-cache-study-ultra-long-qwen05b-merged-r5
```

## Result

The merged artifact has `10` paired shared/control observations for `n=16`.

- control direct counter hit rate: `0.822%`
- shared direct counter hit rate: `92.294%`
- shared-minus-control direct counter delta: `91.472 pp`, interval
  `91.372 pp` to `91.581 pp`
- p95 first-event/TTFT ratio delta: `-1.032`, interval `-1.372` to `-0.848`
- throughput-ratio delta: `2.184`, interval `1.387` to `2.967`
- p95 latency-ratio delta: `-0.809`, interval `-1.063` to `-0.601`
- p95 stream TPOT-ratio delta: `-0.773`, interval `-0.857` to `-0.629`

Generated artifacts:

```text
results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-chunk-r3-seed1401/prefix-cache-isolated-metrics.json
results/modal-vllm-prefix-cache-ultra-long-qwen05b-n16-merged-r5/prefix-cache-isolated-metrics.json
...
# Training 071: Qwen 1.5B Mega-Long L4 n32 Merged r8

Training 071 adds one more three-repeat Qwen 1.5B n32 L4 chunk and merges it
with the Training 070 r5 artifact. This gives the batch-pressure axis the same
repeat count as the strongest n16 Qwen 1.5B L4 model-size artifact.

## Goal

Turn the n32 batch-pressure result into a repeat-count-matched r8 artifact and
check whether the capacity-pressure timing signal remains stable enough to use
as a backend-comparison control point.
...
modal run modal_app.py --mode vllm-prefix-cache-isolated-neutral-warmup \
  ...
  --prefix-cache-shared-profile shared_prefix_mega_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_mega_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-chunk-r3-seed2501
```
...
```bash
modal run modal_app.py --mode vllm-prefix-cache-isolated-merge \
  --prefix-cache-isolated-merge-dirs results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-merged-r5,results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-chunk-r3-seed2501 \
  --prefix-cache-shared-profile shared_prefix_mega_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_mega_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-merged-r8

modal run modal_app.py --mode vllm-prefix-cache-isolated-stability-summary \
  --prefix-cache-isolated-metrics-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-merged-r8 \
  --prefix-cache-shared-profile shared_prefix_mega_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_mega_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-merged-r8-summary

python3 scripts/build_prefix_cache_study_table.py \
  --summary-json results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-merged-r8-summary/prefix-cache-isolated-stability-summary.json \
  --output-dir results/prefix-cache-study-mega-long-qwen15b-l4-n32-merged-r8
```
...
- control direct counter hit rate: `0.446%`
- shared direct counter hit rate: `96.243%`
- shared-minus-control direct counter delta: `95.797 pp`, interval
  `95.748 pp` to `95.845 pp`
- p95 first-event/TTFT ratio delta: `-0.942`, interval `-0.957` to `-0.924`
- throughput-ratio delta: `9.863`, interval `9.015` to `10.711`
- p95 latency-ratio delta: `-0.911`, interval `-0.925` to `-0.897`
- p95 stream TPOT-ratio delta: `-0.683`, interval `-0.736` to `-0.632`
...
# Training 075: Qwen 1.5B L4 n32 Scheduler-Control Smoke

Training 075 adds `max_num_batched_tokens` override support to the paired
prefix-cache benchmark path and runs the n32 Qwen 1.5B L4 smoke with the
restored `60,640` scheduler budget.

## Goal

Test whether the capacity knob identified in Training 074 changes measured
timing behavior, especially the weaker n32 stream TPOT result.
...
modal run modal_app.py --mode vllm-prefix-cache-isolated-neutral-warmup \
  ...
  --output-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-smoke-r2

modal run modal_app.py --mode vllm-prefix-cache-isolated-stability-summary \
  --prefix-cache-isolated-metrics-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-smoke-r2 \
  --prefix-cache-shared-profile shared_prefix_mega_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_mega_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-smoke-r2-summary

python3 scripts/build_prefix_cache_study_table.py \
  --summary-json results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-smoke-r2-summary/prefix-cache-isolated-stability-summary.json \
  --output-dir results/prefix-cache-study-mega-long-qwen15b-l4-n32-batched-tokens60640-smoke-r2
```

## Capacity Check

The Modal console repeatedly showed the controlled scheduler budget and restored
allocator profile:
...
The persisted paired rows capture the requested scheduler knob as
`engine_max_num_batched_tokens=60640`. Exact GPU KV-token allocation still
requires the subprocess capacity diagnostic because vLLM's multiprocessing logs
are not fully visible to the in-process snapshot.
...
- shared-minus-control direct counter delta: `95.869 pp`, interval
  `95.868 pp` to `95.870 pp`
- p95 first-event/TTFT ratio delta: `-0.961`, interval `-0.966` to `-0.956`
- throughput-ratio delta: `10.029`, interval `9.572` to `10.485`
- p95 latency-ratio delta: `-0.922`, interval `-0.930` to `-0.914`
- p95 stream TPOT-ratio delta: `-0.955`, interval `-0.963` to `-0.947`

Compared with the default n32 r2 smoke, the major change is stream TPOT:
`-0.675` at `121,280` max batched tokens becomes `-0.955` at `60,640`.
That is the first timing artifact supporting the hypothesis that the n32 TPOT
weakening was scheduler-capacity-driven, not just a request-count effect.
...
# Training 077: Qwen 1.5B L4 n32 Scheduler-Control Merged r8

Training 077 promotes the `max_num_batched_tokens=60640` n32
scheduler-control axis from r5 to r8. It adds one three-repeat chunk, then
merges it with the Training 076 r5 artifact.

## Goal

Check whether the restored-capacity TPOT finding holds at repeat-count parity
with the default n32 and n16 r8 artifacts.
...
modal run modal_app.py --mode vllm-prefix-cache-isolated-neutral-warmup \
  ...
  --prefix-cache-control-profile matched_unique_prefix_mega_long_no_repeat_variant \
  --prefix-cache-max-num-batched-tokens 60640 \
  --output-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-chunk-r3-seed3001

modal run modal_app.py --mode vllm-prefix-cache-isolated-merge \
  --prefix-cache-isolated-merge-dirs results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-merged-r5,results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-chunk-r3-seed3001 \
  --prefix-cache-shared-profile shared_prefix_mega_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_mega_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-merged-r8

modal run modal_app.py --mode vllm-prefix-cache-isolated-stability-summary \
  --prefix-cache-isolated-metrics-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-merged-r8 \
  --prefix-cache-shared-profile shared_prefix_mega_long_no_repeat_variant \
  --prefix-cache-control-profile matched_unique_prefix_mega_long_no_repeat_variant \
  --output-dir results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-merged-r8-summary

python3 scripts/build_prefix_cache_study_table.py \
  --summary-json results/modal-vllm-prefix-cache-mega-long-qwen15b-l4-n32-batched-tokens60640-merged-r8-summary/prefix-cache-isolated-stability-summary.json \
  --output-dir results/prefix-cache-study-mega-long-qwen15b-l4-n32-batched-tokens60640-merged-r8
```

## Capacity Check

The new r3 chunk repeatedly logged the restored capacity profile:
...
The merged source manifest records the r5 source plus the seed `3001` r3 chunk,
for `16` scenario-level cold/cache paired runs and `8` shared/control
comparisons.

## Result

The merged r8 artifact keeps the controlled scheduler-budget effect stable.

- shared-minus-control direct counter delta: `95.821 pp`, interval
  `95.772 pp` to `95.869 pp`
- p95 first-event/TTFT ratio delta: `-0.954`, interval `-0.960` to `-0.947`
- throughput-ratio delta: `9.184`, interval `7.959` to `10.196`
- p95 latency-ratio delta: `-0.916`, interval `-0.926` to `-0.902`
- p95 stream TPOT-ratio delta: `-0.937`, interval `-0.954` to `-0.918`

Against the default n32 r8 artifact, direct-cache and latency behavior are
similar, throughput is lower (`9.184` controlled versus `9.863` default), and
stream TPOT is much stronger (`-0.937` controlled versus `-0.683` default).
That keeps the claim narrow but useful: controlling vLLM's scheduler token
budget restores the decode-side TPOT behavior under the n32 request shape.
...
Use this controlled r8 artifact as the vLLM baseline for backend comparison or
serving-path comparison. The next useful question is whether another backend
keeps the direct-cache and TPOT effects when the KV-capacity budget is known.
...
The `vllm-server-async-paired` mode now selects T4 or L4 with `--modal-gpu` and
accepts `--server-async-max-num-batched-tokens`. Artifacts record:
...
modal run modal_app.py --mode vllm-server-async-paired \
  ...
  --output-dir results/modal-vllm-server-async-qwen15b-l4-n32-batched-tokens60640-smoke-r1
...
modal run modal_app.py --mode vllm-server-async-paired \
  ...
  --phase-order server_first \
  --server-async-max-num-batched-tokens 60640 \
  --output-dir results/modal-vllm-server-async-qwen15b-l4-n32-batched-tokens60640-server-first-smoke-r1
...
Both phase orders record `modal_gpu=L4`, `max_model_len=3648`,
`max_num_batched_tokens=60640`, `default_max_num_batched_tokens=116736`, and
`max_num_batched_tokens_source=override`. The Modal allocator logs showed:
...
The phase-order smoke has two scenarios, one repeat per order, and two paired
server-vs-`AsyncLLM` rows per order.

- async-first mean server/async throughput ratio: `11.844`
- server-first mean server/async throughput ratio: `12.081`
- async-first mean server/async p95 latency ratio: `0.085`
- server-first mean server/async p95 latency ratio: `0.083`
- async-first mean server/async p95 stream TPOT ratio: `0.020`
...
```text
results/modal-vllm-server-async-qwen15b-l4-n32-batched-tokens60640-smoke-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-n32-batched-tokens60640-smoke-r1/paired-server-async-summary.csv
results/modal-vllm-server-async-qwen15b-l4-n32-batched-tokens60640-smoke-r1/paired-server-async-runs.csv
results/modal-vllm-server-async-qwen15b-l4-n32-batched-tokens60640-server-first-smoke-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-n32-batched-tokens60640-server-first-smoke-r1/paired-server-async-summary.csv
...
# Training 093 - Server KV-Budget Floor Probe

## Goal

Push the fixed-`n=32` KV-budget sweep below Training 092:

```text
Does shared-prefix server KV-cache benefit collapse when the reported KV
capacity is only about 10 full-length requests for a 32-request batch?
```

This checkpoint keeps the Training 092 workload shape and adds
`gpu_memory_utilization=0.35`. It also rebuilds the combined pressure curve
across `0.45`, `0.40`, and `0.35` so the budget trend is visible in one table.

## Commands

The four new measured cells use this shape:
...
modal run modal_app.py --mode vllm-server-async-paired \
  ...
  --output-dir results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu035-n32-batched-tokens60640-{matched-unique|shared-prefix}-seed3805-cache-{off|on}-r1
```

The new off/on pairs are compared with
`vllm-server-async-cache-control-compare`, then the `0.45`, `0.40`, and `0.35`
compare artifacts are combined with
`vllm-server-async-cache-control-server-absolute`, and the final pressure table
is generated with:

```bash
python3 scripts/build_server_cache_pressure_curve.py \
  --server-absolute-json results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-gpu040-gpu035-n32-batched-tokens60640-seed3805-cache-control-r1/server-cache-control-absolute.json \
  --output-dir results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-gpu040-gpu035-n32-batched-tokens60640-seed3805-pressure-curve-r1
```

## Result

The new `gpu_memory_utilization=0.35` logs report:

```text
matched-unique: 34,762 KV tokens, 9.55 max concurrency at 3,639 tokens/request
shared-prefix:  34,809 KV tokens, 9.59 max concurrency at 3,628 tokens/request
```

Direct raw server cache-on divided by cache-off:

```text
matched-unique gpu=0.35:
  prompt pressure: 3.298
  cache-on hit rate: 0.439%
  output tokens/s ratio: 0.992x
  p95 latency ratio: 1.008x

shared-prefix gpu=0.35:
  prompt pressure: 3.295
  cache-on hit rate: 96.109%
  output tokens/s ratio: 9.109x
  p95 latency ratio: 0.110x
```

All four new measured cells completed. Across the combined `0.45`, `0.40`, and
`0.35` curve, matched-unique averages `0.981x` cache-on/cache-off throughput,
while shared-prefix averages `8.740x`.

## Interpretation

This is the strongest pressure result so far. The server handles n32 even when
the reported maximum concurrency is only about 9.6 full-length requests, and
shared-prefix caching still gives an order-of-magnitude class latency and
throughput gain.

The curve is not monotonic: shared-prefix throughput ratio is `9.010x` at
pressure `0.978`, `8.100x` at pressure `1.509`, and `9.109x` at pressure
`3.295`. Treat that as evidence that this single-seed curve is affected by
chunked-prefill scheduling, JIT/warmup noise, or server timing variance. It is
not yet a clean transition curve.

The next useful step is replication, not only more pressure. A good follow-up
is to repeat the `gpu_memory_utilization=0.35` point with one or two new
scenario seeds. If the shared-prefix result stays near `8x-9x`, the research
claim becomes much stronger.

Generated artifacts:

```text
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu035-n32-batched-tokens60640-matched-unique-seed3805-cache-off-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu035-n32-batched-tokens60640-matched-unique-seed3805-cache-on-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu035-n32-batched-tokens60640-matched-unique-seed3805-cache-control-r1/cache-control-phase-order-compare.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu035-n32-batched-tokens60640-shared-prefix-seed3805-cache-off-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu035-n32-batched-tokens60640-shared-prefix-seed3805-cache-on-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu035-n32-batched-tokens60640-shared-prefix-seed3805-cache-control-r1/cache-control-phase-order-compare.json
...
# Training 096 - Complete Replicated KV-Budget Curve

## Goal

Repeat the Training 092 `gpu_memory_utilization=0.45` point with seed `3906`:

```text
Can we produce a two-seed replicated pressure curve across the full current
0.45, 0.40, and 0.35 KV-budget sweep?
```

This checkpoint completes the second-seed pass for the current L4 n32 budget
sweep.
...
modal run modal_app.py --mode vllm-server-async-paired \
  ...
  --output-dir results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-n32-batched-tokens60640-{matched-unique|shared-prefix}-seed3906-cache-{off|on}-r1
```

The seed `3906` off/on pairs are compared with
`vllm-server-async-cache-control-compare`. The `0.45` seed `3805` and seed
`3906` compare artifacts are then combined with
`vllm-server-async-cache-control-server-absolute`.

The full replicated pressure curve is generated by combining all six off/on
compare artifacts across `0.45`, `0.40`, and `0.35`, then running:

```bash
python3 scripts/build_server_cache_pressure_curve.py \
  --server-absolute-json results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-gpu040-gpu035-n32-batched-tokens60640-seed3805-seed3906-cache-control-r1/server-cache-control-absolute.json \
  --output-dir results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-gpu040-gpu035-n32-batched-tokens60640-seed3805-seed3906-pressure-curve-r1
```

## Result

The replicated `gpu_memory_utilization=0.45` logs report:

```text
matched-unique: 117,359 KV tokens, 32.17 max concurrency at 3,648 tokens/request
shared-prefix:  117,006 KV tokens, 32.17 max concurrency at 3,637 tokens/request
```
...
```text
matched-unique:
  cache-on/off throughput contrast: -0.021x
  cache-on/off latency contrast: +0.017x

shared-prefix:
  cache-on/off throughput contrast: +9.423x
  cache-on/off latency contrast: -0.787x
```

The two-seed aggregate at `gpu_memory_utilization=0.45` is:

```text
matched-unique:
  output tokens/s ratio: 0.995x
  bootstrap p05/p95: 0.987x to 1.003x

shared-prefix:
  output tokens/s ratio: 9.125x
  bootstrap p05/p95: 9.010x to 9.241x
```

The full two-seed pressure curve is:

```text
matched-unique gpu=0.35 pressure=3.298 throughput=0.995x p95 latency=1.006x
matched-unique gpu=0.40 pressure=1.510 throughput=0.984x p95 latency=1.016x
matched-unique gpu=0.45 pressure=0.979 throughput=0.995x p95 latency=1.005x
shared-prefix  gpu=0.35 pressure=3.303 throughput=8.467x p95 latency=0.119x
shared-prefix  gpu=0.40 pressure=1.512 throughput=8.638x p95 latency=0.116x
shared-prefix  gpu=0.45 pressure=0.981 throughput=9.125x p95 latency=0.109x
```

Across all 12 trial contrasts, matched-unique averages `0.991x`
cache-on/cache-off throughput, while shared-prefix averages `8.744x`.

## Interpretation

The current KV-budget curve is now replicated across two prompt seeds. The
control profile stays near neutral at every budget point, while the shared
profile keeps an order-of-magnitude class throughput and latency improvement
from pressure `0.98` through pressure `3.30`.

This is a substantially stronger artifact than the earlier single-seed curve.
It supports the central claim that reusable prefix KV blocks can remain highly
valuable even when the reported KV capacity is far below the aggregate prompt
tokens in the batch.

The next useful step is no longer just replication of this curve. Better next
steps are either a lower-budget failure-bound probe, such as
`gpu_memory_utilization=0.30`, or adding a stronger instrumentation layer that
records per-run scheduler/cache counters in a report-ready table.

Generated artifacts:

```text
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-n32-batched-tokens60640-matched-unique-seed3906-cache-off-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-n32-batched-tokens60640-matched-unique-seed3906-cache-on-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-n32-batched-tokens60640-matched-unique-seed3906-cache-control-r1/cache-control-phase-order-compare.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-n32-batched-tokens60640-shared-prefix-seed3906-cache-off-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-n32-batched-tokens60640-shared-prefix-seed3906-cache-on-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-n32-batched-tokens60640-shared-prefix-seed3906-cache-control-r1/cache-control-phase-order-compare.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu045-n32-batched-tokens60640-seed3805-seed3906-cache-control-r1/server-cache-control-absolute.json
...
# Training 099 - Replicate Near-Floor KV-Budget Point

## Goal

Repeat Training 098 with seed `3805`:

```text
Does the floor-adjacent 0.325 point replicate, or was the 12x shared-prefix
speedup a single-seed outlier?
```

This checkpoint makes every successful budget point in the current curve
two-seed replicated: `0.325`, `0.35`, `0.40`, and `0.45`.
...
modal run modal_app.py --mode vllm-server-async-paired \
  ...
  --server-async-prefix-caching {off|on} \
  --gpu-memory-utilization 0.325 \
  --output-dir results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-{matched-unique|shared-prefix}-seed3805-cache-{off|on}-r1
```

The seed `3805` off/on pairs are compared with
`vllm-server-async-cache-control-compare`. The `0.325` seed `3805` and seed
`3906` compare artifacts are then combined with
`vllm-server-async-cache-control-server-absolute`. The final four-budget curve
uses all off/on compare artifacts from `0.325`, `0.35`, `0.40`, and `0.45`.

## Result

The replicated `gpu_memory_utilization=0.325` logs report:

```text
matched-unique seed=3805: 14,188 KV tokens, 3.90 max concurrency at 3,639 tokens/request
shared-prefix seed=3805:  14,208 KV tokens, 3.92 max concurrency at 3,628 tokens/request
```

Seed `3805` direct raw server cache-on divided by cache-off:

```text
matched-unique:
  cache-on/off throughput contrast: -0.039x
  cache-on/off latency contrast: +0.030x

shared-prefix:
  cache-on/off throughput contrast: +10.161x
  cache-on/off latency contrast: -0.766x
```

The two-seed aggregate for `gpu_memory_utilization=0.325` is:

```text
matched-unique:
  prompt pressure: 8.080
  output tokens/s ratio: 0.975x
  bootstrap p05/p95: 0.968x to 0.983x

shared-prefix:
  prompt pressure: 8.091
  output tokens/s ratio: 10.741x
  bootstrap p05/p95: 9.025x to 12.456x
```

The fully replicated four-budget curve is:

```text
matched-unique gpu=0.325 pressure=8.080 throughput=0.975x p95 latency=1.026x
matched-unique gpu=0.350 pressure=3.298 throughput=0.995x p95 latency=1.006x
matched-unique gpu=0.400 pressure=1.510 throughput=0.984x p95 latency=1.016x
matched-unique gpu=0.450 pressure=0.979 throughput=0.995x p95 latency=1.005x
shared-prefix  gpu=0.325 pressure=8.091 throughput=10.741x p95 latency=0.095x
shared-prefix  gpu=0.350 pressure=3.303 throughput=8.467x p95 latency=0.119x
shared-prefix  gpu=0.400 pressure=1.512 throughput=8.638x p95 latency=0.116x
shared-prefix  gpu=0.450 pressure=0.981 throughput=9.125x p95 latency=0.109x
```

Across all 16 trial contrasts, matched-unique averages `0.987x`
cache-on/cache-off throughput, while shared-prefix averages `9.243x`.

## Interpretation

The floor-adjacent point replicated. The shared-prefix effect remains larger
than the rest of the curve at `0.325`, but it is no longer a single-run result:
the second seed still gives a double-digit cache-on/cache-off throughput
contrast in the direct comparison, and the two-seed server-only aggregate is
`10.741x`.

The practical lower bound is now clear for this workload shape: `0.30` fails
before measurement, while `0.325` initializes and preserves a strong cache
benefit at roughly `8.1x` estimated prompt pressure. This is a strong candidate
for the headline stress-test point in the writeup.

The next useful step is to stop extending the curve and improve the artifact:
add a report-ready summary table that joins startup floor, KV capacity, prompt
pressure, cache hit rate, throughput ratio, and latency ratio across all
budget points.

Generated artifacts:

```text
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-matched-unique-seed3805-cache-off-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-matched-unique-seed3805-cache-on-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-matched-unique-seed3805-cache-control-r1/cache-control-phase-order-compare.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-shared-prefix-seed3805-cache-off-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-shared-prefix-seed3805-cache-on-r1/paired-server-async.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-shared-prefix-seed3805-cache-control-r1/cache-control-phase-order-compare.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-seed3805-seed3906-cache-control-r1/server-cache-control-absolute.json
results/modal-vllm-server-async-qwen15b-l4-kvbudget-gpu0325-n32-batched-tokens60640-seed3805-seed3906-pressure-curve-r1/server-cache-pressure-curve.md
